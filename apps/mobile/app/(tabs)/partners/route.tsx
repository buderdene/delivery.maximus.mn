/**
 * МАРШРУТЫН ХАРИЛЦАГЧИД ДЭЛГЭЦ
 * 
 * BUSINESS RULE:
 * - Борлуулагч бүрт 5 өдрийн маршрут хуваарилагдсан (Да, Мя, Лх, Пү, Ба)
 * - Өнөөдрийн гараг автоматаар сонгогдоно (getTodayAsDay)
 * - Гараг солиход тухайн өдрийн харилцагчид 1C ERP-ээс татагдана
 * 
 * API:
 * - 1C ERP: /hs/ts/Tasks?day={0-4}&username={username}
 * - day=0: Даваа, day=1: Мягмар, ..., day=4: Баасан
 * 
 * TAB ШҮҮЛТҮҮД:
 * - Бүгд: Тухайн өдрийн бүх харилцагч
 * - Ирсэн: coordinateRange=1 ЭСВЭЛ GPS зай <= routeRange
 * - Зай хол: coordinateRange!=1 МӨНӨӨС GPS зай > routeRange
 */
...
import { usePartnerStore, DAY_LABELS, type DayOfWeek } from '../../../stores/partner-store';
...
export default function RoutePartnersScreen() {
  ...
  useEffect(() => {
    fetchPartnersByDay(filters.day);
  }, []);
  ...
  const onRefresh = useCallback(async () => {
    ...
    clearSearch();
    setSearchInput('');
    await fetchPartnersByDay(filters.day);
    setRefreshing(false);
  }, [fetchPartnersByDay, filters.day, clearSearch]);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <Box className="flex-1 bg-background-50">
        {/* Header */}
        <Box className="bg-white px-4 pt-2 pb-2 border-b border-outline-100">
      ...
      {/* Day Tabs */}
      <Box className="bg-white px-4 pt-3 pb-2">
        <Text size="sm" className="text-typography-500 mb-2">Маршрут</Text>
        <HStack space="sm" className="justify-between">
          {DAY_LABELS.map((day) => (
            <TouchableOpacity
              key={day.key}
              onPress={() => setDay(day.key)}
              style={[
                styles.dayTab,
                filters.day === day.key && styles.activeDayTab,
              ]}
            >
              <Text
                size="sm"
                style={{ fontFamily: filters.day === day.key ? 'GIP-SemiBold' : 'GIP-Regular' }}
                className={filters.day === day.key ? 'text-white' : 'text-typography-700'}
              >
                {day.short}
              </Text>
            </TouchableOpacity>
      ...
      {/* Status Tabs */}
      <Box className="bg-white px-4 pt-2 pb-3">
        <HStack space="md">
          {tabs.map((tab) => (
            <TouchableOpacity
              key={tab.key}
      ...
      {error && (
        <Box className="mx-4 mt-2 bg-error-50 p-4 rounded-xl border border-error-200">
          <HStack space="sm" className="items-center">
            ...
            <TouchableOpacity onPress={() => fetchPartnersByDay(filters.day)}>
          ...
          ListEmptyComponent={
            !isLoading && !isSearching ? (
              <Box className="flex-1 justify-center items-center py-20">
                <Box className="bg-amber-50 p-6 rounded-full mb-4">
                ...
                <Text size="md" className="text-typography-500 text-center mt-2 px-8">
                  {searchInput ? 'Хайлтын үр дүн олдсонгүй' : 'Энэ өдөрт харилцагч байхгүй'}
                </Text>
                <Button
                  size="md"
                  variant="outline"
